...
import { CvData } from '../builders/DocxBuilder'
  ...
  const updateBullet = (expIdx: number, bulletIdx: number, value: string) =>
    onChange({
      ...
      experiences: cv.experiences.map((e, i) =>
        i === expIdx
          ? { ...e, bullets: (e.bullets as string[]).map((b, bi) => (bi === bulletIdx ? value : b)) }
          : e
      )
    })
    ...
    <div className='cvp'>
          ...
          return (
            <div key={exp.key} className='cvp-exp'>
              <div className='cvp-exp-meta'>
                ...
                />
                <span className='cvp-exp-sub'>{base.employer} · {base.period}</span>
              </div>
      ...
      {Object.values(ALL_EXPERIENCES).some(e => e.type === 'volunteer') && (
        <div className='cvp-section'>
          <label className='cvp-mind'>
            ...
            />
            <span>Inkludera övrig erfarenhet ({Object.values(ALL_EXPERIENCES).filter(e => e.type === 'volunteer').map(e => e.employer).join(', ')})</span>
          </label>
        </div>
      )}
      ...
      <div className='cvp-section'>
        ...
        {USER.education.map((u, i) => (
          <div key={i} className='cvp-edu'>{u.name} · {u.period}</div>
        ))}
      </div>
